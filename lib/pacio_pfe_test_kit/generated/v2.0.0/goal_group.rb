...
module PacioPFETestKit
  module PacioPFEV200
    class GoalGroup < Inferno::TestGroup
      title 'Personal Functioning and Engagement Goal Tests'
      short_description <<~DESC
        'Verify support for the server capabilities required by the Personal Functioning and Engagement Goal Profile.'
      DESC
      description %(
# Background

The PFE Personal Functioning and Engagement Goal sequence verifies that the system under test is
able to provide correct responses for Goal queries. These queries
must contain resources conforming to the Personal Functioning and Engagement Goal Profile as
...
read succeeds.
      ...
      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(
          YAML.load_file(File.join(__dir__, 'goal', 'metadata.yml'), aliases: true)
        )
      end

      test from: :pfe_v200_goal_patient_search_test
      test from: :pfe_v200_goal_subject_search_test
      test from: :pfe_v200_goal_target_date_search_test
      test from: :pfe_v200_goal_lifecycle_status_search_test
      test from: :pfe_v200_goal_patient_target_date_search_test
      test from: :pfe_v200_goal_patient_description_search_test
      test from: :pfe_v200_goal_patient_lifecycle_status_search_test
