...
The PFE Patient sequence verifies that the system under test is
...
read succeeds.
      ...
      id :pfe_v300_ballot_patient
      run_as_group

      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(
          YAML.load_file(File.join(__dir__, 'patient', 'metadata.yml'), aliases: true)
        )
      end

      test from: :pfe_v300_ballot_patient__id_search_test
      test from: :pfe_v300_ballot_patient_identifier_search_test
      test from: :pfe_v300_ballot_patient_name_search_test
      test from: :pfe_v300_ballot_patient_birthdate_name_search_test
      test from: :pfe_v300_ballot_patient_read_test
      test from: :pfe_v300_ballot_patient_validation_test
      test from: :pfe_v300_ballot_patient_must_support_test
    end
  end
end
